...
if (BingNiaoSDK::DEBUG) API::log($p, 'bingniao_pay', 'request');
//参数验证
if(empty($p['extinfo'])) {
    API::log(array('msg' => '缺少extinfo参数内容'), 'bingniao_pay', 'request');
    BingNiaoSDK::out(1);
}
//透传数据解析
...
if((int)$status !== 1) {
    API::log(array('msg' => $info['cps'] . '标签验证status状态不为1'), 'bingniao_pay', 'request');
    BingNiaoSDK::out(1);
}
//sign验证
if($p['sign'] !== BingNiaoSDK::createPaySign($p, $info['cps'])) {
    API::log(array('msg' => $info['cps'] . '标签验证sign错误'), 'bingniao_pay', 'request');
    BingNiaoSDK::out(1);
}
...
if(!API::checkPayAccount($info['rid'], $info['platform'], $info['zone_id'], $acc)) {
    API::log('充值账号跟角色注册帐号不匹配', 'bingniao_pay', 'error');
    BingNiaoSDK::out(1);
}

$api = new PayApi($info['rid'], $info['platform'], $info['zone_id']);
$ret = $api->pay($order_no, $amount, $ext);
if(BingNiaoSDK::DEBUG) API::log($ret, 'bingniao_pay', 'pay_ret');
switch ($ret['error']) {
    case 'SUCCESS':
    case 'ORDER_EXISTS':
        BingNiaoSDK::out(0);
